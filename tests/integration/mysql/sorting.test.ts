/**
 * MySQL — orderBy and distinct.
 *
 * Covers:
 *   - single-field orderBy (asc / desc)
 *   - multi-field orderBy
 *   - orderBy on a relation field
 *   - distinct (single and multi-column)
 *
 * Re-executes each captured SELECT to confirm both syntactic validity and row
 * equivalence with Prisma's own result.
 */
...
describe('MySQL — orderBy', () => {
  ...
  it('multiple fields', async () => {
    // Add a second row with the same name to verify the tiebreaker field is used.
  ...
  it('relation field (order customers by their order count via _count)', async () => {
    // Give customer "Bravo" two orders, "Alpha" one. Then orderBy { orders: { _count: 'desc' } }.
...
describe('MySQL — distinct', () => {
  ...
  // NOTE: Prisma applies `distinct` in JavaScript after fetching rows, rather
  // than emitting SQL DISTINCT. So the logged SQL — which is what we
  // interpolate — does *not* contain DISTINCT and returns the raw rows. Our
  // job here is to verify the SQL we log is still syntactically valid and
  // executable; deduplication semantics belong to Prisma's runtime.
  ...
  it('distinct on multiple columns — logged SQL executes', async () => {
    const events = await captureQueries(() =>
      prisma.customer.findMany({
        distinct: ['name', 'email'],
        orderBy: { email: 'asc' },
