/**
 * PostgreSQL — end-to-end test of `createPrismaSqlLogger`.
 *
 * Mirrors `tests/integration/mysql/create-logger.test.ts` but with
 * `dialect: 'postgresql'`, so that the public API surface is exercised against
 * both dialects rather than just MySQL.
 *
 * Verifies that:
 *   1. A custom logger receives the interpolated SQL + `{ duration }` meta.
 *   2. The logged SQL is executable via `$queryRawUnsafe` and returns the same
 *      row Prisma's original query returned.
 *   3. The default `console.log` logger prepends `(Nms)` when `showDuration`
 *      is enabled.
 */
...
describe('createPrismaSqlLogger (end-to-end with real Prisma, PostgreSQL)', () => {
  beforeAll(async () => {
    prisma = createPrisma();
  });
  ...
  it('logs runnable SQL for a real query via custom logger', async () => {
    ...
    // No leftover $N placeholders in the interpolated SQL.
    expect(insertLog!.sql).not.toMatch(/\$\d/);
  ...
  it('logged SQL can be executed against the same database', async () => {
    ...
    // Re-execute the logged SQL as raw and expect the same row count
  ...
  it('showDuration prepends (Nms) prefix with default logger', async () => {
    ...
    try {
      const log = createPrismaSqlLogger({ dialect: 'postgresql', showDuration: true });
      prisma.$on('query', log);
