...
function toSql(event: QueryEvent): string {
  return interpolate(event.query, parseParams(event.params), 'postgresql');
}
...
describe('PostgreSQL integration', () => {
  ...
  describe('query event shape', () => {
    ...
    it('uses $N placeholders for PostgreSQL', async () => {
      ...
      expect(selectEvent!.query).toMatch(/\$\d+/);
      expect(selectEvent!.query).not.toContain('?');
    });
  });
  ...
  describe('interpolated SQL returns identical results', () => {
    ...
    it('SELECT by boolean', async () => {
      ...
      expect(replayed.length).toBe(original.length);
      // PostgreSQL has a native boolean type, so the raw query returns true/false.
      expect(replayed[0]!['bool']).toBe(true);
    });
    ...
    it('SELECT findFirst by id returns identical row', async () => {
      ...
      expect(row['bool']).toBe(created.bool);
  ...
  /**
   * Replay all captured SELECT queries against PostgreSQL.
